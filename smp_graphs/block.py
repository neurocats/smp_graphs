...
from smp_graphs.common import dict_get_nodekeys_recursive, \
    dict_replace_nodekeys_loop
from smp_graphs.common import dict_search_recursive, \
...
class decStep():
    ...
    def wrap_l1(self, xself, f, *args, **kwargs):
        ...
        if not self.get_credit(xself) or self.process_blk_mode(
            xself): return None
    ...
    def process_input(self, xself):
        ...
        for k, v in list(xself.inputs.items()):
            ...
            # check input sanity
            assert v['val'].shape == v[
                'shape'], "%s-%s's real and desired input shapes need to agree but ink = %s, %s != %s" % (
            ...
            if 'bus' in v:  # input item is driven by external signal (bus value)
                ...
                if blocksize_input_bus == v['shape'][
                    -1]:  # for all vshape in 1,2,3..
                ...
                elif blocksize_input_bus > v['shape'][
                    -1]:  # for all vshape in 1,2,3..
                    ...
                    if xself.cnt % xself.blocksize == 0:
                        ...
                        sls = (
                              xself.cnt - v['shape'][-1]) % blocksize_input_bus
                        sle = sls + v['shape'][
                            -1]  # xself.cnt % blocksize_input_bus
                        sl = slice(sls, sle)  #
                    ...
                    if (xself.cnt % blocksize_input) == 0:
                        ...
                        try:
                            ...
                            v['val'][..., -blocksize_input_bus:] = xself.bus[
                                v['bus']].copy()
                        except Exception as e:
                            print((
                                "%s-%s[%d].decStep input copy k = %s from bus %s/%s to %s/%s, %s" % (
                                xself.cname, xself.id, xself.cnt, k, v['bus'],
                                xself.bus[v['bus']].shape, v['shape'],
                                v['val'].shape, e)))
                            sys.exit(1)
    ...
    def process_output(self, xself):
        ...
        if xself.block_is_scheduled():
            ...
            for k, v in list(xself.outputs.items()):
                ...
                assert xself.bus[v['buskey']].shape == v[
                    'shape'], "%s-%s's real and desired output shapes need to agree, but self.%s = %s != %s output['shape']" % (
                    xself.cname, xself.id, k, xself.bus[v['buskey']].shape,
                    v['shape'])
                ...
                if v['buskey'] == 'robot1/h':
                    logger.debug(
                        "output '%s', xself.bus[v['buskey'] = %s] = %s / %s" % (
                        k, v['buskey'], xself.bus[v['buskey']],
                        getattr(xself, k)))
                ...
                if xself.logging:
                    ...
                    log.log_pd(tbl_name=v['buskey'],
                               data=xself.bus[v['buskey']])
                    # except:
        ...
        if xself.block_is_finished():
            ...
            def update_block_store(block=None, top=None):
                ...
                print(("update_block_store block = %s (%s)" % (block,
                                                              block.md5)))
            ...
            if not xself.topblock and xself.top.docache and xself.cache is not None and xself.cache_loaded:  # cache_block and cache_inst:
                ...
                for outk, outv in list(xself.outputs.items()):
                    ...
                    if self.output_is_type(outk, outv):
                        continue
                    setattr(xself, outk, xself.cache_data[outk][
                                         xself.cnt - xself.blocksize:xself.cnt,
                                         ...].T)
...
class Block2(object):
    ...
    defaults = {
        ...
        'cnt': 1,
    # 1, FIXME: log / cache issues: replaced init_cnt = 1 with init_cnt = 0 and topblock.step() once after graph init?
        'blocksize': 1,  # period of computation calls in time steps
        'blockphase': [0],
    # list of positions of comp calls along the counter in time steps
    ...
    def __init__(self, conf={}, paren=None, top=None, blockid=None,
        ...
        if type(self.conf) == dict and 'params' in self.conf:
            ...
            print()
            "What could it be? Look at %s" % (self.conf)
        ...
        self.logger = get_module_logger(
            modulename='{0}.{1}'.format(self.modulename, self.cname_full),
    ...
    def init_block(self):
        ...
        if self.block_is_composite():
            ...
            self._info("%s%s-%s.init_block composite" % (
            self.nesting_indent, self.cname, self.id))
    ...
    def node_cloning(self):
        if True:
            ...
            if hasattr(self, 'graph') \
                ...
                # search node
                print()
                "top graph", self.top.nxgraph.nodes()
                targetid = self.graph[3:]  # id template
                targetnode = nxgraph_node_by_id_recursive(self.top.nxgraph,
                                                          targetid)
                print()
                "%s-%s" % (self.cname,
                           self.id), "targetid", targetid, "targetnode", targetnode
                if len(targetnode) > 0:
                    print()
                    "    targetnode id = %d, node = %s" % (
                        targetnode[0][0],
                ...
                for k in list(tnode.keys()):
                    print()
                    "cloning: subcloning: k = %s, v = %s" % (k, tnode[k])
                ...
                for k, v in list(clone['block_'].inputs.items()):
                    ...
                    if hasattr(self, 'inputs'):
                        ...
                        v['bus'] = re.sub(id_orig, clone['params']['id'],
                                          v['bus'])
                    clone['block_'].inputs[k] = copy.deepcopy(v)
                    print()
                    "%s.init cloning  input k = %s, v = %s" % (
                    self.cname, k, clone['block_'].inputs[k])

                # replace output refs
                for k, v in list(clone['block_'].outputs.items()):
                    # v['buskey'].split("/")[0], v['buskey'].split("/")[0] + "_clone"
                    v['buskey'] = re.sub(id_orig, clone['params']['id'],
                                         v['buskey'])
                    print()
                    "%s.init cloning output k = %s, v = %s" % (
                    self.cname, k, v)
                    clone['block_'].outputs[k] = copy.deepcopy(v)
                print()
                "cloning: cloned block_.id = %s" % (clone['block_'].id)
    ...
    def init_block_cache(self):
        ...
        def check_block_store(block=None, top=None):
            ...
            assert hasattr(top,
                           'block_store'), "Top block needs to have a pandas.HDF5Store type 'block_store' attribute"
            ...
            self._debug("block_store has blocks? is %s" % (
            hasattr(top.block_store, 'blocks'),))
            ...
            if len(list(top.block_store.keys())) > 0:
                ...
                try:
                    ...
                    print()
                    "%s%s-%s.check_block_store cache retrieval for %s failed with %s" % (
            ...
            if top.docache and block.cache is not None and block.cache.shape[
                0] != 0:
                print()
                "%s%s-%s.check_block_store" % (
                block.nesting_indent, block.cname, block.id)
                print()
                ...
                if top.cache_clear:
                    logstr = "%s    cache_clear set, deleting cache entry %s / %s" % (
                    block.nesting_indent, block.md5, blocks.shape)
                    self._debug(logstr)
                    ...
                    if hits is not None:
                        ...
                        for hit in hits.index[hits]:
                            ...
                            print()
                            "%s    cache_clear set, dropping, pre = %s, post = %s" % (
                            block.nesting_indent, blocks_pre_drop_shape,
                            blocks.shape)
                    # # save
                    ...
                    if top.log_store_cache is None:
                        top.log_store_cache = pd.HDFStore(
                            block.cache['log_store'].values[0])

                    block.cache_data = {}
                    for outk, outv in list(block.outputs.items()):
                        if 'type' in outv and outv[
                            'type'] != 'ndarray': continue
                        # x_ = block.cache_h5['%s/%s' % (block.id, outk)].values
                        x_ = top.log_store_cache[
                            '%s/%s' % (block.id, outk)].values
                        print()
                        "%s%s-%s.check_block_store:     loading output %s cached data = %s" % (
                            block.nesting_indent,
            ...
            if top.docache and block.cache is None:
                ...
                columns = [
                    'md5', 'timestamp', 'block', 'params', 'log_store',
                    'nxgraph']  # 'experiment',
                values = [[
                ...
                if len(list(top.block_store.keys())) < 1:
                    ...
                    blocks = top.block_store.blocks
                    print()
                    "%scache None found, no cache entries exist %s, %s" % (
                        block.nesting_indent, blocks.shape, df.shape)
                else:
                    print()
                    "%s    cache None found, cache entries exist %s" % (
                        block.nesting_indent, blocks.shape)
                    print()
                    "%s    cache_new blocks pre  concat = %s" % (
                    block.nesting_indent, blocks.shape)
                    blocks = pd.concat([blocks, df])
                    print()
                    "%s    cache_new blocks post concat = %s" % (
                    block.nesting_indent, blocks.shape)
                    # print "df2 concat(store, df)", df2.shape, df2
                    #  top.block_store['/blocks'] = df2
                    print()
                    "%s    cache_new inserted entry at index %s with md5 = %s" % (
                        block.nesting_indent, index,
                        block.md5)  # , df.md5, df2.shape)
            ...
            print()
            "%s    cache block_store final shape = %s" % (
                block.nesting_indent, blocks.shape,)
    ...
    def init_subgraph(self):
        ...
        # subgraph file containing the OrderedDict
            ...
            if hasattr(self, 'lconf'):
                ...
                subconf_vars = get_config_raw(conf=self.subgraph, confvar=None,
                                              lconf=self.lconf)
                # print "init_subgraph %s returning localvars = %s" % (self.id, subconf_vars.keys())
                subconf = subconf_vars['conf']
            else:
                subconf_vars = get_config_raw(self.subgraph, confvar=None,
                                              lconf=self.top.conf_vars)  # 'graph')
                subconf = subconf_vars['conf']
        ...
        assert subconf['params'][
                   'numsteps'] <= self.top.numsteps, "enclosed numsteps = %d greater than top level numsteps = %d" % (
        subconf['params']['numsteps'], self.top.numsteps)

        # set the graph params
        self.conf['params']['graph'] = copy.deepcopy(
            subconf['params']['graph'])
        ...
        if hasattr(self, 'subgraphconf'):
            ...
            for confk, confv in list(self.subgraphconf.items()):
                ...
                confnode = dict_search_recursive(self.conf['params']['graph'],
                                                 confk_id)
                # return on fail
                if confnode is None: continue
                print()
                "subgraphconf node = %s" % (confnode['block'],)
                print()
                "               param = %s" % (confk_param,)
                print()
                "               val_bare = %s" % (confv,)
                ...
                if type(confv) is dict:
                    ...
                    print()
                    "               val_new = %s, tmp_ = %s" % (
                    confnode['params'][confk_param], tmp_)
                else:
                    print()
                    "               val_new = %s" % (confv,)
        ...
        if hasattr(self, 'subgraph_rewrite_id') and self.subgraph_rewrite_id:
            ...
            self._debug("    conf['params']['id'] = %s, xid_ = %s" % (
            self.conf['params']['id'], xid_,))
            if len(xid_) == 1:
                xid = '0'
            elif xid_[-1].isdigit():
                ...
                xid = (
                    '_' + '_'.join(
                        [item for item in xid_ if item[:2] == loop_delim[1:]]),
    ...
    def init_graph_pass_1(self):
        ...
        self._info("{2}{0: <20}-{3}.init_graph_pass_1 graph.keys = {1}".format(
            self.cname[:20], self.nxgraph.nodes(), self.nesting_indent,
            self.id))
        ...
        for i in nxgraph_nodes_iter(self.nxgraph, 'enable'):
            ...
            # debug timing
            self._info(
                "{3}{0: <20}.init pass 1 k = {1: >5}, v = {2: >20}".format(
                    self.__class__.__name__[:20], k[:20], v['block'].__name__,
                    self.nesting_indent))
            then = time.time()
            ...
            now = time.time()
            self._info(
                "{3}{0: <20}.init pass 1 k = {1: >5}, v = {2: >20}, took = {5}s".format(
                    self.__class__.__name__[:20], k, v['block_'].cname,
    ...
    def init_graph_pass_2(self):
        ...
        for i in nxgraph_nodes_iter(self.nxgraph, 'enable'):
            ...
            self._info(
                "{3}{0: <20}.init pass 2 k = {1: >5}, v = {2: >20}".format(
                    self.__class__.__name__[:20], k, v['block_'].cname,
                    self.nesting_indent))
            then = time.time()
            ...
            self._info(
                "{3}{0: <20}.init pass 2 k = {1: >5}, v = {2: >20}, took = {4}s".format(
                    self.__class__.__name__[:20], k, v['block_'].cname,
                    self.nesting_indent, time.time() - then))
    ...
    def init_outputs(self):
        ...
        for k, v in list(self.outputs.items()):  # problematic
            ...
            assert type(
                v) is dict, "Old config of block %s, output %s, type %s, %s" % (
            self.id, k, type(v), v)
    ...
    def init_outputs_ndarray(self, k=None, v=None):
        ...
        assert 'shape' in v, "%s-%s's output spec %s needs 'shape' param but has %s " % (
        self.cname, self.id, k, list(v.keys()))
        # if v.has_key('shape'):
        assert len(v[
                       'shape']) > 1, "Block %s, output %s 'shape' tuple is needs at least (dim1 x output blocksize), v = %s" % (
        self.id, k, v)
        ...
        if hasattr(self, 'ros') and self.ros:
            ...
            self.pubs[k] = rospy.Publisher('%s/%s' % (self.id, k,),
    ...
    def init_logging(self):
        ...
        for k, v in list(self.outputs.items()):
            if ('init' not in v) or (not v['init']) or (
            not v['logging']): continue

            # FIXME: ellipsis
            tbl_columns_dims = "_".join(["%d" for axis in v['shape'][:-1]])
            tbl_columns = [tbl_columns_dims % tup for tup in
                           xproduct(itertools.product, v['shape'][:-1])]
            ...
            log.log_pd_init_block(
                ...
                numsteps=(
                         self.top.numsteps / self.blocksize) * self.oblocksize,
    ...
    def init_pass_2(self):
        ...
        if not self.topblock:
            ...
            for k, v in list(self.inputs.items()):
                self._debug(
                    "init_pass_2 input items ink = %s, inv = %s" % (k, v,))
                assert len(v) > 0
                ...
                if 'bus' in v:
                    if 'shape' in v:
                        ...
                        if self.inputs_clamp:
                            v['shape'] = (v['shape'][0], min(self.top.numsteps,
                                                             v['shape'][1]))
                        ...
                        if v['bus'] not in self.bus:
                            ...
                            for i in range(1):  # 5
                                ...
                                self._warning(
                                    "%s%s-%s init (pass 2) WARNING: nonexistent bus %s" % (
                                    self.cname, self.id, v['bus'],
                                    self.nesting_indent))
                            ...
                            if blocksize_input_bus > v['shape'][-1]:
                                # mcnt = xself.cnt % blocksize_input_bus
                                # sl = slice(mcnt - v['shape'][-1] + 1, mcnt + 1) #
                                sls = (self.cnt - v['shape'][
                                    -1]) % blocksize_input_bus
                                sle = sls + v['shape'][
                                    -1]  # xself.cnt % blocksize_input_bus
                                sl = slice(sls, sle)  #
                                ...
                                assert v['shape'][0] == \
                                       self.bus[v['bus']].shape[
                                           0], "%s-%s's input buffer and input bus shapes need to agree (besides blocksize) for input %s, buf: %s, bus: %s/%s" % (
                                self.cname, self.id, k, v['shape'], v['bus'],
                                self.bus[v['bus']].shape)
                                v['val'][..., -blocksize_input_bus:] = \
                                self.bus[v['bus']].copy()
                        ...
                        assert v['bus'] in self.bus, "Requested by %s-%s, bus item %s is not in buskeys %s" % (
                        self.cname, self.id, v['bus'], list(self.bus.keys()))
                    ...
                    self._debug(
                        "init_pass_2 input items ink = %s, bus[%s] = %s, input = %s" % (
                        k, v['bus'], self.bus[v['bus']].shape, v['val'].shape))
                ...
                self._debug(
                    "init_pass_2 input items ink = %s inv['val'].shape/inv['shape'] = %s / %s" % (
                    k, v['val'].shape, v['shape']))
    ...
    def step(self, x=None):
        ...
        if self.topblock:
            ...
            if (self.cnt) % 500 == 0 or self.cnt == (
                self.numsteps - 1) or self.cnt == (
                self.numsteps - self.blocksize_min):
                self._info(
                    "storing log @iter % 4d/%d" % (self.cnt, self.numsteps))
                log.log_pd_store()
    ...
    def bus_copy(self):
        ...
        """
        for k, v in [(k_, v_) for k_, v_ in list(self.outputs.items()) if
                     'buscopy' in v_]:
            buskey = v['buscopy']
            assert buskey in self.bus, "Assuming in %s-%s that bus has key %s but %s" % (
            self.cname, self.id, buskey, list(self.bus.keys()))
            ...
            self._debug("    buscopy outputs[%s] from bus[%s] = %s" % (
            k, buskey, self.bus[buskey]))
    ...
    def latex_close(self):
        ...
        output_latex = [(k, v) for k, v in list(self.outputs.items()) if
                        'type' in v and v['type'] == 'latex']
        if len(output_latex) < 1:
            self._info(
                "latex_close: no latex output configured, output_latex = %s" % (
                output_latex,))
            return
        ...
        output_figures = [(k, v) for k, v in list(self.outputs.items()) if
                          'type' in v and v['type'] in ['fig', 'plot']]
	...
	\\caption{\\label{fig:%s}}\n\  \\end{subfigure}\n" % (figv['filename'], ref, )
        ...
        if len(output_figures) > 0:
            for figk, figv in output_figures:
                ...
                c_ = ', '.join(
                    ["%s \\autoref{fig:%s}" % (desc, ref) for (desc, ref) in
                     zip(descs, refs)])
    ...
    def plot_close(self):
        ...
        for n in nxgraph_nodes_iter(self.nxgraph, 'enable'):
            ...
            if hasattr(node, 'saveplot'):
                self._debug("%s-%s.plot_close examining node %s (%s)" % (
                self.cname, self.id, node.id, node.saveplot))

                # if type(node) is Block2 and hasattr(node, 'saveplot') and node.saveplot:
                if node.saveplot:
                    self._debug(
                        "%s-%s.plot_close closing node, saving plot %s" % (
                        self.cname, self.id, node.id,))
                    try:
                        node.save()
                    except Exception as e:
                        logger.error(
                            '%s-%s.plot_close node(%s-%s).save() failed with %s' % (
                            self.cname, self.id, node.cname, node.id, e))
        ...
        for i in nxgraph_nodes_iter(self.nxgraph, 'enable'):
            ...
            for k, v in list(node.outputs.items()):
                ...
                log.log_pd_init_block_attr(
                    ...
                    tbl_dim=v['shape'],
                    numsteps=(
                             node.top.numsteps / node.blocksize) * node.oblocksize,
    ...
    def dump_final_config(self):
        ...
        self.numsteps, confstr,) + conf_footer
        # print confstr_
        f.write(confstr_)
        f.flush()
    ...
    def check_attrs(self, attrs):
        ...
        for attr in attrs:
            assert hasattr(self,
                           attr), "%s.check_attrs: Don't have attr = %s" % (
    ...
    def get_input(self, k):
        ...
        if k not in self.inputs:
            logger.error(
                '%s-%s get_input failed, no key = %s in self.inputs.keys = %s' % (
                self.cname, self.id, list(self.inputs.keys())))
            return np.zeros((1, 1))
        ...
        if 'embedding' in self.inputs[k]:
            ...
            for i in range(embshp[1]):
                ...
                tmp = np.hstack(tuple(
                    self.inputs[k]['val'][:, i - j] for j in range(emblen)))
...
class LoopBlock2(Block2):
    ...
    def subgraph_from_loop_unrolled(blockref, conf, paren, top):
        ...
        print()
        "        - nxgraph_from_smp_graph loopblock %s unroll %s" % (
            conf['params']['id'],
            conf['params']['loop'],)
        ...
        for k, v in list(loopgraph_unrolled.items()):
            print()
            "loop %s, params = %s" % (k, list(v['params'].keys()))
        # sys.exit(1)
...
class SeqLoopBlock2(Block2):
    ...
    def step_cache(self, x=None):
        ...
        print()
        "%s-%s[%d] this should never print but be caught by step decorator :)" % (
        self.cname, self.id, self.cnt)
        print()
        "%s-%s[%d] is cached at %s\n    cache = %s" % (
        self.cname, self.id, self.cnt, self.md5, self.cache)
        if isinstance(self, SeqLoopBlock2):  # hasattr(self, 'cache_data') and
            print()
            "    ready for batch playback of %s" % (list(self.cache_data.keys()),)
        for outk in list(self.outputs.keys()):
            print()
            "cached self.%s = %s" % (outk, getattr(self, outk))
            pass
    ...
    def step_compute(self, x=None):
        ...
        def f_obj(lparams):
            ...
            loopblock_conf = {'block': self.loopblock['block'],
                              'params': copy.deepcopy(loopblock_params)}
            ...
            self.confgraph['%s' % loopblock_params['id']] = copy.copy(
                loopblock_conf)
        ...
        def f_obj_hpo(params):
            ...
            # XXX
            lparams = (
            'inputs', {'x': {'val': x}})  # , x.shape, self.outputs['x']
            self._debug(
                "step[%d] f_obj_hpo lparams = %s" % (self.cnt, lparams,))
            ...
            for outk in list(self.outputs.keys()):
                ...
                loss += np.mean(getattr(self.dynblock, outk), axis=1,
                                keepdims=True)
        ...
        for i in range(self.numsteps / self.loopblocksize):
            ...
            self._debug(
                "step[%d] loop iter %d, trying to get results, time = %s" % (
                self.cnt, i, then))
            results = self.f_loop(i, f_obj_)
            # print "results", results
            self._debug(
                "step[%d] f_loop results[%d] = %s" % (self.cnt, i, results,))
            ...
            for outk in list(self.outputs.keys()):
                ...
                if outk not in self.dynblock.outputs:
                    self._warning(
                        "Output %s not found in %s-%s's outputs %s" % (
                        outk, self.dynblock.cname, self.dynblock.id,
                        list(self.dynblock.outputs.keys())))
                    continue

                # compute slice
                outslice = slice(i * self.dynblock.outputs[outk]['shape'][-1],
                                 (i + 1) *
                                 self.dynblock.outputs[outk]['shape'][-1])
        ...
        confgraph_full = {'block': Block2,
                          'params': {'id': self.id, 'graph': self.confgraph}}

        # print "%s-%s.step dynamic graph = %s" % (self.cname, self.id, confgraph_full)
        self.nxgraph = nxgraph_from_smp_graph(confgraph_full)

        for outk in list(self.outputs.keys()):
            logstr = "step[%d/%d] output %s = %s" % (
            self.top.cnt, self.cnt, outk, getattr(self, outk))
            self._debug(logstr)
...
class IBlock2(PrimBlock2):
    ...
    def __init__(self, conf={}, paren=None, top=None):
        ...
        for k, v in list(conf['params']['inputs'].items()):
            print()
            "%s.init inkeys %s" % (self.__class__.__name__, k)
            print()
            "IBlock2 conf['params'] keys", list(conf['params'].keys())
            print()
            "IBlock2 conf['params']['outputs'] keys = %s" % (
            list(conf['params']['outputs'].keys()),)  # ["I%s" % k]
            outk = 'I%s' % (k,)
            ...
            if outk in conf['params']['outputs']:
                ...
                conf['params']['outputs'][outk] = {'shape': v[
                    'shape']}  # {'shape': top.bus[busk['bus']].shape} # ['val'].shape]}
    ...
    def step_leak(self, x=None):
        for i in range(self.blocksize):
            for ink in list(self.inputs.keys()):
                ...
                tmp_[:, i] = ((1 - self.leak) * tmp_[:, i - 1]) + (
                self.inputs[ink][0][:, i] * self.d)
    ...
    def step_all(self, x=None):
        for ink, inv in list(self.inputs.items()):
            ...
            print()
            "IBlock2.step[%d] self.%s = %s / %s" % (
            self.cnt, outk, getattr(self, outk).shape,
            self.outputs[outk]['shape'])
...
class dBlock2(PrimBlock2):
    ...
    def step(self, x=None):
        ...
        for ink in list(self.inputs.keys()):
            ...
            print()
            "dBlock2.step", self.id, self.cnt, getattr(self, outk)[:,
                                               [-1]].shape, self.inputs[ink][
                'val'].shape, din.shape
...
class DelayBlock2(PrimBlock2):
    ...
    def __init__(self, conf={}, paren=None, top=None):
        ...
        for ink, inv in list(params['inputs'].items()):
            ...
            bufshape = (
            inshape[0], inshape[1] + (delay_max + 1))  # delay_max) #
        ...
        for k, v in list(self.delays.items()):
            ...
            delay_tap_bs = (
            delay_tap - np.tile(np.array(list(range(blocksize_input, 0, -1))),
                                (delay_tap.shape[0], 1)).T).T
            if self.flat:
                delay_tap_bs = delay_tap_bs.flatten()
            self.delaytaps[k] = delay_tap_bs.copy()
            self._debug('delay %s with taps %s added delaytaps_bs = %s' % (
            k, v, self.delaytaps[k]))
    ...
    def step(self, x=None):
        ...
        for ink in list(self.inputs.keys()):
            ...
            if self.cnt % blocksize_input not in self.blockphase:
                self._debug(
                    'input %s[%d] deferred for shape end = %s overriding blocksize = %s' % (
                    ink, self.cnt, blocksize_input, self.blocksize))
                continue
            ...
            if blocksize_input != self.blocksize:
                sl = slice(-blocksize_input, None)
                self._debug('input %s shape end = %s, blocksize = %s' % (
                ink, self.inputs[ink]['shape'][-1], self.blocksize))
            ...
            if blocksize_input == self.top.numsteps:
                self._debug(
                    'input %s bsi > numsteps, slice sl.start = %s, sl.stop = %s, inv_.shape = %s' % (
                    ink, sl.start, sl.stop, inv_.shape))
                # cyclic wrap of input chunk
                inv_[..., slice(None, sl.start)] = inv_[
                    ..., slice(-sl.start, None)]
        ...
        for k, v in list(params['inputs'].items()):
            ...
            for slk, slv in list(slicespec.items()):
                ...
                if type(slv) is slice:
                    params['outputs'][outk] = {'shape': (slv.stop - slv.start,
                                                         oblocksize)}  # top.bus[params['inputs'][k][0]].shape
                elif type(slv) is list:
                    params['outputs'][outk] = {'shape': (len(slv),
                                                         oblocksize)}  # top.bus[params['inputs'][k][0]].shape
                elif type(slv) is tuple:
                    params['outputs'][outk] = {'shape': (slv[1] - slv[0],
                                                         oblocksize)}  # top.bus[params['inputs'][k][0]].shape
...
class ConstBlock2(PrimBlock2):
    ...
    def __init__(self, conf={}, paren=None, top=None):
        ...
        if self.inputs['c']['val'].shape[1] == 1:
            self.x = np.tile(self.inputs['c']['val'],
                             self.blocksize)  # FIXME as that good? only works for single column vector
        else:
            self.x = self.inputs['c'][
                'val'].copy()  # FIXME as that good? only works for single column vector
...
class CountBlock2(PrimBlock2):
    ...
    def step(self, x=None):
        ...
        # if self.blocksize > 1:
        if outshapenum > 1:
            newcnt = np.tile(np.arange(self.cnt - outshapenum, self.cnt),
                             outshape[:-1]).reshape(outshape)
...
class UniformRandomBlock2(PrimBlock2):
    ...
    def step(self, x=None):
        self._debug("step:\n\tx = %s,\n\tinputs = %s,\n\toutputs = %s" % (
        list(self.outputs.keys()), self.inputs, self.outputs))
        ...
        if self.cnt % self.rate == 0:
            ...
            for k, v in list(self.outputs.items()):
                ...
                x = np.random.uniform(self.inputs['lo']['val'],
                                      self.inputs['hi']['val'],
                                      size=v['shape'])
                setattr(self, k, x)
